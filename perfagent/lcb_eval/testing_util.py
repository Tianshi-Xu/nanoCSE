...
def run_test(sample, test=None, debug=False, timeout=6):
    ...
    # reliability_guard() 会将 os 多处设为 None，同进程内后续代码会报错，run_test 结束时需恢复。
    import os as _os
    _saved_putenv = getattr(_os, "putenv", None)
    _saved_getcwd = getattr(_os, "getcwd", None)
    _saved_chdir = getattr(_os, "chdir", None)
    ...
    try:
        ...
        # 恢复 os 属性，避免同进程内后续报错（putenv→environ 赋值；getcwd/chdir→logging.FileHandler→abspath）
        _os.putenv = _saved_putenv
        _os.getcwd = _saved_getcwd
