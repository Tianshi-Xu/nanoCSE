    ...
    def _validate_and_prepare_paths(self) -> dict[str, Any]:
        ...
        if "instance_templates_dir" in config:
            ...
            if instance_templates_dir.exists() and any(instance_templates_dir.glob("*.yaml")):
                ...
                print(
                    f"Instance templates directory {instance_templates_dir} not found or empty, using default system prompt"
                )
...
def main():
    ...
    parser.add_argument("--validate-only", action="store_true", help="Only validate configuration without running")
