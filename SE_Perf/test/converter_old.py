...
class TrajectoryReorganizer:
    ...
    def _get_next_run_number(self, instance_dir: Path) -> int:
        ...
        existing_runs = [int(d.name) for d in instance_dir.iterdir() if d.is_dir() and d.name.isdigit()]
    ...
    def _create_tra_file(self, traj_file: Path, tra_file: Path) -> dict[str, int]:
        ...
        try:
            ...
            return {"total_tokens": total_tokens, "history_items": len(simplified_history)}
    ...
    def reorganize(self) -> None:
        ...
        for instance_dir in instance_dirs:
            ...
            files_to_organize = [f for f in instance_dir.iterdir() if f.is_file() and f.name != "traj.pool"]
            ...
            for file_path in files_to_organize:
                ...
                if file_path.suffix == ".traj":
                    ...
                    print(
                        f"Created {tra_file.name} with {stats['history_items']} history items, "
        ...
        print(f"\nSummary: Processed {total_files} trajectory files, ~{total_tokens} total tokens")
...
def main():
    ...
    parser.add_argument(
        "config", nargs="?", default="Aeon/run/configs/test_claude.yaml", help="Path to configuration YAML file"
    )
