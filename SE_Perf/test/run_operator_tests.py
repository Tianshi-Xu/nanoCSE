...
class SETestSuite:
    ...
    def run_unified_data_interface_test(self):
        ...
        try:
            result = subprocess.run([sys.executable, str(test_script)], capture_output=True, text=True, timeout=120)
            ...
            print("❌ 测试超时")
            self.test_results["unified_data_interface"] = {"success": False, "description": "统一数据接口 - 测试超时"}
            return False
        except Exception as e:
            print(f"❌ 测试执行失败: {e}")
            self.test_results["unified_data_interface"] = {"success": False, "description": "统一数据接口 - 执行失败"}
            return False
    ...
    def run_operator_data_access_test(self):
        ...
        try:
            ...
            print("❌ 测试超时")
            self.test_results["operator_data_access"] = {"success": False, "description": "算子数据访问 - 测试超时"}
            return False
        except Exception as e:
            print(f"❌ 测试执行失败: {e}")
            self.test_results["operator_data_access"] = {"success": False, "description": "算子数据访问 - 执行失败"}
            return False
    ...
    def run_data_format_validation_test(self):
        ...
        try:
            ...
            print(f"❌ 验证失败: {e}")
            self.test_results["data_format_validation"] = {"success": False, "description": "数据格式验证 - 验证失败"}
            return False
    ...
    def run_legacy_compatibility_test(self):
        ...
        try:
            # 测试旧版本测试脚本是否仍然可用
            old_tests = ["test_alternative_strategy.py", "test_traj_pool_summary.py"]
            ...
            print(f"❌ 兼容性测试失败: {e}")
            self.test_results["legacy_compatibility"] = {"success": False, "description": "遗留兼容性 - 测试失败"}
            return False
...
def main():
    ...
    parser.add_argument(
        "--test",
        choices=["unified_data", "operator_access", "format_validation", "legacy_compat", "all"],
        default="all",
        help="指定要运行的测试",
    )
