...
def analyze_patch_with_openai(
    ...
    if is_empty_patch:
        ...
        prompt = prompt_template.format(problem_statement=problem_statement, model_patch=model_patch)
...
def analyze_patch_with_claude(
    problem_statement: str, model_patch: str, instance_id: str, claude_api: ClaudeAPI, is_empty_patch: bool = False
) -> Dict[str, Any]:
    """
    使用Claude API分析补丁
...
def fix_analysis_item(
    ...
    try:
        ...
        if api_type == "claude" and claude_api:
            new_analysis = analyze_patch_with_claude(problem_statement, model_patch, key, claude_api, is_empty_patch)
        elif api_type == "openai":
            new_analysis = analyze_patch_with_openai(problem_statement, model_patch, key, is_empty_patch=is_empty_patch)
        else:
...
def fix_conclusion_file(
    ...
    if test_item:
        if test_item in predictions:
            ...
            key, need_fix, new_analysis = fix_analysis_item(
                test_item, predictions[test_item], api_type, claude_api, rate_limit_delay
            )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_key = {
            executor.submit(fix_analysis_item, key, predictions[key], api_type, claude_api, rate_limit_delay): key
    ...
    if stats["start_time"] and stats["end_time"]:
        ...
        log_message(
            f"统计信息: 总项目数 {stats['total_items']}, 已处理 {stats['processed_items']}, "
...
def main():
    ...
    parser.add_argument(
        "--base_path",
        default="/home/uaih3k9x/swebench/evolve_agent/exp_claude37_0-30",
    ...
    parser.add_argument(
        "--api_type", choices=["claude", "openai"], default="openai", help="使用的API类型：claude或openai(默认)"
    )
    parser.add_argument("--claude_api_key", default=CLAUDE_API_KEY, help="Claude API密钥")
    ...
    parser.add_argument(
        "--rate_limit_delay", type=float, default=1.0, help="API请求之间的延迟时间(秒)，用于避免频率限制"
    )
    ...
    if args.scan_dir:
        if os.path.exists(args.scan_dir):
            ...
            for file_path in conclusion_files:
                fix_conclusion_file(
                    file_path, args.api_type, claude_api, args.test, args.max_workers, args.rate_limit_delay
                )
        else:
    ...
    # 处理每个文件
    for file_path in conclusion_files:
        fix_conclusion_file(file_path, args.api_type, claude_api, args.test, args.max_workers, args.rate_limit_delay)
