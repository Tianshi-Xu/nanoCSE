...
def analyze_patch_with_claude(
    problem_statement: str, model_patch: str, instance_id: str, claude_api: ClaudeAPI
) -> Dict[str, Any]:
    """
    使用Claude API分析补丁
    ...
    while retry_count <= max_retries:
        try:
            response = claude_api.send_message(
                message=prompt, model="claude-3-7-sonnet-20250219", temperature=0.3, max_tokens=4000
            )
...
def process_conclusion_file(
    ...
    for i in range(0, len(entries_to_process), batch_size):
        ...
        print(
            f"正在处理批次 {i // batch_size + 1}/{(len(entries_to_process) + batch_size - 1) // batch_size}，共 {len(batch)} 条"
        )
...
def main():
    ...
    parser.add_argument(
        "--base_path",
        default="/home/uaih3k9x/swebench/evolve_agent/newest_exp_claude37_30-125",
        help="基础路径，包含5个default文件夹",
    )
    parser.add_argument("--test", action="store_true", help="测试模式：只处理每个文件的第一条数据")
    parser.add_argument(
        "--api_type", choices=["claude", "openai"], default="claude", help="使用的API类型：claude或openai(默认)"
    )
    parser.add_argument("--claude_api_key", default="api_key", help="Claude API密钥")
    parser.add_argument("--max_workers", type=int, default=3, help="最大并发线程数，默认为3")
    parser.add_argument("--force", action="store_true", help="强制重新处理已有分析的条目")
    ...
    for file_path in conclusion_files:
        process_conclusion_file(file_path, args.api_type, claude_api, args.test, args.max_workers, args.force)
