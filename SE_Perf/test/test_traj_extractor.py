    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if stats["instances"]:
            ...
            if instance_summary and "problem" in instance_summary:
                problem_preview = (
                    instance_summary["problem"][:100] + "..."
                    if len(instance_summary["problem"]) > 100
                    else instance_summary["problem"]
                )
            ...
            for key, value in instance_summary.items():
                if key != "problem" and key.isdigit():
                    print(
                        f"  迭代{key}: {value.get('approach_summary', 'N/A') if isinstance(value, dict) else str(value)[:50]}"
                    )
