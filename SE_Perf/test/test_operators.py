...
def test_all_operators():
    ...
    # 2. 测试算子创建
    config = {"operator_models": {"name": "openai/deepseek-chat", "temperature": 0.0}}
    ...
    for op_name in expected_operators:
        ...
        # 测试算子处理
        try:
            result = operator.process(workspace_dir=test_workspace, current_iteration=2, num_workers=1)
...
def show_operator_comparison():
    ...
    print("\n💡 使用建议:")
    print("  - 迭代2: 使用 alternative_strategy (基于迭代1的失败)")
    print("  - 迭代3+: 使用 traj_pool_summary (综合分析所有历史)")
