...
class ProblemManager:
    ...
    def get_problem_description(self, instance_path: str, method: str = "auto") -> str | None:
        """
        获取实例的问题描述
        ...
        if method == "auto":
            ...
            return (
                self._get_from_problem_file(instance_path)
                or self._get_from_trajectory(instance_path)
                or self._get_from_json_config(instance_path)
            )
        elif method == "file":
    ...
    def _get_from_problem_file(self, instance_path: Path) -> str | None:
        """从.problem文件获取问题描述"""
    ...
    def _get_from_trajectory(self, instance_path: Path) -> str | None:
        """从轨迹文件提取问题描述"""
        instance_name = instance_path.name
        ...
        try:
            ...
            # 提取PR描述
            if len(trajectory_data) > 1 and "content" in trajectory_data[1] and len(trajectory_data[1]["content"]) > 0:
                text_content = trajectory_data[1]["content"][0].get("text", "")

                # 提取<pr_description>标签内容
                pr_match = re.search(r"<pr_description>(.*?)</pr_description>", text_content, re.DOTALL)
                if pr_match:
    ...
    def _get_from_json_config(self, instance_path: Path) -> str | None:
        """从JSON配置文件获取问题描述（待实现）"""
...
def get_problem_description(instance_path: str, method: str = "auto") -> str | None:
    """便捷函数：获取问题描述"""
